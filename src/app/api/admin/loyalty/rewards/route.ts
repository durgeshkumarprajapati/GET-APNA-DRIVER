...
export const GET = withPermission(PERMISSIONS.ADMIN_LOYALTY_READ, async (req) => {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to load customer rewards catalog';
    return NextResponse.json({ success: false, error: 'REWARDS_FETCH_FAILED', message }, { status: 500 });
  }
});
...
export const POST = withPermission(PERMISSIONS.ADMIN_LOYALTY_MANAGE, async (req) => {
  try {
    ...
    return NextResponse.json({ success: false, error: 'REWARD_CREATE_FAILED', message }, { status: 500 });
  }
});
