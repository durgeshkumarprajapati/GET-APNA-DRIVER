...
import { archivePromotion } from '@/modules/promotion/application/services/promotion-service';
...
export const POST = withPermission<RouteParams>(
  ...
  async (_req, { principal }, routeContext) => {
    ...
    const promotion = await archivePromotion(principal.userId, promotionId);
