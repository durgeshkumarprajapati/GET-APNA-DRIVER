...
export const POST = withPermission(
  ...
  async (req: NextRequest, _context, routeContext?: { params: Promise<{ riskId: string }> }) => {
    try {
      ...
      const updated = await RiskDecisionService.updateStatus(riskId, 'ACKNOWLEDGED', operatorId);
      ...
      return NextResponse.json({
        success: true,
        message: `Risk decision '${riskId}' acknowledged.`,
        decision: updated,
      });
    } catch (err: unknown) {
      return NextResponse.json(
        {
          success: false,
          error: err instanceof Error ? err.message : 'Failed to acknowledge risk decision',
        },
        { status: 500 },
      );
    }
