...
export const GET = withPermission<RouteParams>(
  ...
  async (_req, { principal }, routeContext) => {
    try {
      ...
      const reliability = await service.getCustomerReliabilityView(principal.userId, bookingId);
      ...
      const message = err instanceof Error ? err.message : 'Failed to fetch customer trip reliability.';
