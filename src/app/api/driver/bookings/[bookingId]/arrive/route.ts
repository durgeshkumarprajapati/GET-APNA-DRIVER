...
import { markArrived } from '@/modules/booking/application/driver-journey-service';
import {
...
export const POST = withPermission<RouteParams>(
  ...
  async (_req, { principal }, routeContext) => {
    try {
      ...
      const booking = await markArrived(principal.userId, bookingId);
      return NextResponse.json(
        { booking, message: 'Driver has arrived at pickup location.' },
        { status: 200 },
      );
    } catch (err: unknown) {
