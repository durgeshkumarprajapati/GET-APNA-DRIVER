...
import { completeTrip } from '@/modules/booking/application/driver-journey-service';
import {
...
export const POST = withPermission<RouteParams>(
  ...
  async (_req, { principal }, routeContext) => {
    try {
      ...
      const booking = await completeTrip(principal.userId, bookingId);
      return NextResponse.json(
        { booking, message: 'Trip completed successfully.' },
        { status: 200 },
      );
    } catch (err: unknown) {
