...
import { startEnRoute } from '@/modules/booking/application/driver-journey-service';
import {
...
export const POST = withPermission<RouteParams>(
  ...
  async (_req, { principal }, routeContext) => {
    try {
      ...
      const booking = await startEnRoute(principal.userId, bookingId);
      return NextResponse.json({ booking, message: 'Driver is now en route.' }, { status: 200 });
    } catch (err: unknown) {
