...
export const GET = withPermission(
  ...
  async (req: NextRequest, { principal }) => {
    try {
      ...
      const userRole = principal.roles?.[0] || 'DRIVER';
      ...
      const result = await callingService.listUserCallHistory(principal.userId, userRole, {
        page,
        limit,
      });
      ...
      const message = err instanceof Error ? err.message : 'Failed to fetch driver call history.';
