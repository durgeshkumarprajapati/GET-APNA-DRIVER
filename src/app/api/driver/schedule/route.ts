...
const upsertWeeklyScheduleSchema = z.object({
  entries: z.array(
    z.object({
      ...
      startTime: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, 'startTime must be in 24-hour HH:mm format'),
      endTime: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, 'endTime must be in 24-hour HH:mm format'),
    ...
    try {
      ...
      if (err instanceof z.ZodError) {
        return NextResponse.json(
          { error: 'VALIDATION_ERROR', message: 'Invalid schedule parameters', details: err.issues },
          { status: 400 },
        );
      }
