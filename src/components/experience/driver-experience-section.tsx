...
export function DriverExperienceSection() {
  ...
  const loadExperiences = useCallback(async () => {
    try {
      const res = await fetch('/api/driver/experience');
      const data = await res.json();
      if (res.ok && data.success) {
        setRecommendations(data.experiences || []);
        setError(null);
      } else {
        setError(data.error || 'Failed to load driver experiences');
      }
    } catch {
  ...
  const handleDismiss = async (rec: ExperienceRecommendation) => {
    ...
    try {
      await fetch(`/api/driver/experience/${rec.id}/dismiss`, {
        method: 'POST',
      ...
      console.error('Failed to dismiss driver experience:', err);
    }
  };
  ...
  return (
    <div className="w-full space-y-2.5">
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-bold text-[#68dba9] uppercase tracking-wider font-['Space_Grotesk']">
          Cockpit Signals & Shift Incentives
        </span>
