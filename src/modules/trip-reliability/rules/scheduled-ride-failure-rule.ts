...
export function evaluateScheduledRideFailure(input: RuleEvaluationInput): RuleEvaluationResult | null {
  if (input.scheduledRideMissedDispatch) {
    return {
      ...
      severity: 'HIGH',
      confidence: 'HIGH',
      reason: 'Scheduled ride occurrence failed to start dispatch search at scheduled dispatch window.',
